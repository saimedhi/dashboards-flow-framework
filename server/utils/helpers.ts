...
export function getClientBasedOnDataSource(
  ...
  console.log("getClientBasedOnDataSource - dataSourceEnabled", dataSourceEnabled);
  console.log("getClientBasedOnDataSource - dataSourceId", dataSourceId);

  if (dataSourceEnabled && dataSourceId && dataSourceId.trim().length != 0) {
    console.log("getClientBasedOnDataSource - if dataSourceEnabled && dataSourceId", context);
    // client for remote cluster
    return context.dataSource.opensearch.legacy.getClient(dataSourceId).callAPI;
  } else {
    console.log("getClientBasedOnDataSource - else");
